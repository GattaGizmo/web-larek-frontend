...
Данный класс реализует модальное окно. Использует методы для управления состоянием модального окна (открытие, закрытие, рендеринг). В этот класс передается любой контент, который будет рендериться в модальном окне. Также устанавливаются слушатели для того, чтобы модальное окно закрывалось по клику на крестик, оверлею или при нажатии Esc.
...
Конструктор **constructor(container: HTMLElement, protected events: IEvents)** принимает селектор для нахождения модального окна в DOM и экземпляр брокера событий IEvents для работы с событиями.

Этот класс только управляет состоянием окна и добавляет переданный контент (будь то форма, успех, ошибка или что-то еще) в модальное окно.

### *SuccessView*

Является представлением для отображения сообщения об успешном заказе и включает:
...
- render(): HTMLElement — рендерит представление и возвращает HTML-элемент. Он может создавать или обновлять разметку с информацией об успешном заказе (стоимость, кнопка для перехода на главную страницу и т. д.).

### *FormView* 

Предназначен для работы с модальным окном, которое содержит форму с полями ввода. 
...
Методы:
- get form: HTMLFormElement - геттер, который получает элемент формы.
...
- render(): HTMLElement — рендерит форму и возвращает HTML-разметку. Этот метод будет генерировать разметку формы с полями для ввода, кнопкой и местом для вывода ошибок.
